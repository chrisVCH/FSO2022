...
router.post('/:id/comments', async (request, response) => {
  ...
  const addedComment = blog.comments.concat(comment)
  const updatedBlog = await Blog.findByIdAndUpdate(request.params.id,  { title, url, author, likes, comments: addedComment }, { new: true })
  response.json(updatedBlog)
})
